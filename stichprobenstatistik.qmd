...
format:
  buechlein-html:
    include-in-header: 
      text: |
        <img src="https://vg08.met.vgwort.de/na/ddff99b0d50b4599871ba688d503781b" width="1" height="1" alt="">
---
...
Mithilfe der relativen Häufigkeitstabelle ist der Median der kleinste Wert mit einer kumulativen absoluten Häufigkeit $N_i$ größer oder gleich 13 bzw. einer 
...
# blaue Hilfslinien
lines(c(170, 170), c(0, 34),  col="skyblue", lty=3, lwd=1.3)
lines(c(0, 170),   c(34, 34), col="skyblue", lty=3, lwd=1.3)
lines(c(180, 180), c(0, 70),  col="skyblue", lty=3, lwd=1.3)
lines(c(0, 180),   c(70, 70), col="skyblue", lty=3, lwd=1.3)
...
# Strahlensatz1
lines(c(170, 174.44), c(10, 10), col="green", lty=4, lwd=1.7)
lines(c(170, 180),    c(20, 20), col="green", lty=4, lwd=1.7)
# Strahlensatz2
...
$$
\begin{aligned}
\text{Median} &= 170 + (180 - 170) \cdot \frac{50 - 34}{70 - 34}\\
 &= 170+ 10\cdot \frac{4}{9} \approx 174,44\\
\end{aligned}
$$
...
hist(x,
     ...
     prob=T, 
     xlab = " ", 
     main=expression("mesokurtische Verteilung " * g[2]==0), 
     ylab=expression("relative Häufigkeit " * f[i]), 
     col="skyblue", 
     axes=F)
axis(1,
...
hist(x, 
     ...
     ylim=c(0,0.2), 
     xlab ="", 
     main=expression("platykurtische Verteilung " * g[2]<0), 
     ylab=expression("relative Häufigkeit " * f[i]), 
     col="skyblue", 
     axes=F)
axis(1,
...
hist(x, 
     ...
     ylim=c(0,0.4), 
     xlab ="", 
     main=expression("leptokurtische Verteilung " * g[2]>0), 
     ylab=expression("relative Häufigkeit " * f[i]), 
     col="skyblue", 
     axes=F)
axis(1,
...
Normalverteilungen sind symmetrisch und mesokurtisch, weshalb ihre Symmetriekoeffizienten und Kurtosiskoeffizienten den Wert 0 haben. 
Daher ist eine Möglichkeit, eine Variable auf Normalverteilung zu testen, die Abweichungen 
der Skewness- und Kurtosis-Koeffizienten von 0 zu betrachten.  
...
#### Standardisierung und z-Transformation
...
\mbox{Student:} &  Hans & Tina & Kim & Flo & Karl\\\hline
Mathe: & 2 & 5 & 4 & \cellcolor{coral} \color{white}\mathbf{8} & 6 & \qquad & \bar x = 5 & \quad s_x = 2\\
Musik: & 1 & 9 &\cellcolor{coral} \color{white} \mathbf{8} & 5 & 2 & \qquad & \bar y = 5 & \quad s_y = 3.16\\
\end{array}
$$
...
Es mag den Anschein haben, dass Kim und Flo die gleiche Leistung in den Fächern erbracht haben, da sie die 
gleichen Leistungspunkte erzielen. 
Aber um die Leistung der Studierenden zum Rest der Gruppe zu bestimmen, müssen wir die Abweichungen der Leitungspunkte in jedem Fach betrachten. Aus diesem Grund ist es besser, die $z$-Werte als Maßstab für die
...
$$
\begin{array}{cccccc}
Mathe: & -1.5 & 0 & -0.5 & \cellcolor{coral} \color{white}\mathbf{1.5} & 0.5 \\
Musik: & -1.26 & 1.26 &\cellcolor{coral} \color{white} \mathbf{0.95} & 0 & -0.95\\
\end{array}
$$
...
\mbox{Student:} &  Hans & Tina & Kim & Flo & Karl\\\hline
Mathe: & 2 & 5 & 4 & 8 & 6 \\
Musik: & 1 & 9 & 8 & 5 & 2 \\ \hline
\sum & 3 &\cellcolor{coral} \color{white} \mathbf{14} & 12 & 13 & 8
\end{array}
$$
...
\begin{array}{rccccc}
\mbox{Student:} &  Hans & Tina & Kim & Flo & Karl\\\hline
Mathe: & -1.5 & 0 & -0.5 & 1.5 & 0.5 \\
Musik: & -1.26 & 1.26 & 0.95 & 0 & -0.95\\ \hline
\sum & -2.76 & 1.26 & 0.45 &\cellcolor{coral} \color{white} \mathbf{1.5} & -0.45
\end{array}
$$
...
par(mfrow = c(1, 3), mar=c(1,1,1,1))  # kleiner Rand für alle
...
# 2. Mittelpanel mit Pfeil
plot.new()
plot.window(xlim = c(0, 1), ylim = c(0, 1))
arrows(0.1, 0.5, 0.9, 0.5, length = 0.1, lwd = 2)
text(0.5, 0.6, expression(Y == X^2), cex = 2)

# 3. Rechtes Histogramm
x2 <- rnorm(1000000)
hist(x2,
     xlim = c(-3.5, 3.5), 
     breaks = 50, 
...
#| label: transformlogx
#| fig-height: 3
# Set up plot area with 3 panels side by side
par(mfrow = c(1, 3), mar=c(1,1,1,1))  # kleiner Rand für alle
...
plot.window(xlim = c(0, 1), ylim = c(0, 1))
arrows(0.1, 0.5, 0.9, 0.5, length = 0.1, lwd = 2)
text(0.5, 0.6, expression(Y == log(X)), cex = 2)
...
options(digits=1)
par(cex.lab=1.2)
out <- histbackback(split(data$height, data$sex), 
                    xlim=c(-10,10),
                    main = 'Histogram der Körpergröße nach Geschlecht', 
                    xlab=c("Frauen", "Männer"), 
                    ylab="Körpergröße")
abline(v= (-8:8)*2 , col ="gray" , lty =3) 
barplot(-out$left, col="salmon" , horiz=TRUE, space=0, add=TRUE, axes=FALSE) 
...
par(cex.lab=1.2)
boxplot(height~sex, 
        main="Häufigkeitsverteilung Körpergröße nach Geschlecht", 
        ylab="Körpergröße", 
        xlab="Geschlecht", 
        ylim=c(150,200), 
        labels=c("Männer","Frauen"), 
        col=c("salmon", "skyblue"), data=data)
