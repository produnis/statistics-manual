...
- **Klassifizierung ohne Gruppieren**: Sortieren der Werte vom kleinsten zum größten (wenn eine Reihenfolge existiert). Diese Methode wird bei qualitativen Variablen und diskreten Variablen mit wenigen eindeutigen Werten verwendet.  
- **Klassifizierung mit Gruppieren**: Gruppieren der Werte in Intervalle (Klassen) und Sortieren dieser Intervalle vom kleinsten  zum größsten. Diese Methode wird bei kontinuierlichen Variablen und diskreten Variablen mit vielen eindeutigen Werten verwendet.
...
1. **Absolute Häufigkeit ($h_{i}$):**  ist die Anzahl, mit der ein bestimmter Wert $x_{i}$ in der Stichprobe auftritt.
...
f_{i} = \frac{h_{i}}{n}
$$
3. **Kumulative absolute Häufigkeit ($H_{i}$):** ist die Gesamtanzahl der Werte in der Stichprobe, die kleiner oder gleich $x_{i}$ sind
...
|:---:|:---:|:---:|:---:|:---:|
| $x_{1}$| $h_{1}$| $f_{1}$| $H_{1}$| $F_{1}$ |
| $\vdots$ | $\vdots$ | $\vdots$ | $\vdots$ | $\vdots$ |
| $x_{i}$| $h_{i}$| $f_{i}$| $H_{i}$| $F_{i}$ |
| $\vdots$ | $\vdots$ | $\vdots$ | $\vdots$ | $\vdots$ |
| $x_{k}$| $h_{k}$| $f_{k}$| $H_{k}$| $F_{k}$ |
...
Häufigkeitsverteilungen werden üblicherweise auch grafisch dargestellt. Abhängig von dem Typ der Variablen und ob die Daten gruppiert wurden, gibt es verschiedene Arten von Diagrammen:  
